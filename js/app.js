...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${images}></img>
      ...
      <div class="d-flex gap-4 mt-4 ms-2">
...
const updatePrice = (id, value) => {
  const getConvertedOldPrice = getInputValue(id);
  const convertPrice = parseFloat(value);
  const convertedOldPrice = parseFloat(getConvertedOldPrice);
  const total =   convertPrice + convertedOldPrice;
